...
|---|---|---|---|---|---|
| Codex | ChatGPT personal/workspace subscription | `codex app-server` ChatGPT-managed browser or device-code login | `auth_protocol_spike` + bounded transport + supervisor contract | `codex-cli 0.146.1`; schema `codex-cli-0.146.1`; executable digest required by driver | Real process and encrypted capsule drivers; durable reconciliation; thread/turn mapping and approval bridge; approved test accounts; AUTH-001–010 on runner; independent isolation review |
...
- Deployment Profile workload and Credential Capsule lifecycle contracts pass PROFILE-008 and PROFILE-010 in the synthetic full profile. The experimental AWS package has real in-cluster EKS/EFS bindings and deterministic AWS/EKS conformance, but no cloud runner or capsule has been created or live-qualified.
...
This is contract, authentication-protocol, transport, and synthetic supervisor evidence, not production-provider support.
...
- [Codex Provider Runner transport](./CODEX_RUNNER_TRANSPORT.md) records the JSONL framing, correlation, fail-closed server-request policy, resource bounds, tests, and remaining runtime isolation work.
- [Codex Provider Runner supervisor](./CODEX_RUNNER_SUPERVISOR.md) records ownership, exact manifest validation, lifecycle, generation fencing, stop/destruction semantics, synthetic conformance, and remaining real-driver work.
