...
**Current Implementation:** the provider-neutral Interface, static policy registry, in-memory connection metadata Implementation, audit sink, and synthetic Adapter exist in `packages/provider-runtime-broker`. AUTH-001 through AUTH-010 pass on that synthetic target. An authentication-only Codex Adapter pins captured `codex-cli 0.146.1` schemas and passes CODEX-001 through CODEX-008 for initialization, device login, raw-response sanitation, account and limit mapping, logout, billing separation, and fail-closed turn execution. A bounded stdio JSONL transport passes RUNNER-001 through RUNNER-008 for correlation, notification separation, error redaction, framing, resource bounds, shutdown, and default rejection of server-initiated requests. A capsule-bound supervisor contract passes SUPERVISOR-001 through SUPERVISOR-008 for exact manifest pins, hidden ownership, initialization health, serialized starts, crash generation fencing, explicit recovery, graceful/forced stop, and irreversible destruction against synthetic runtime/capsule drivers. This proves contract and lifecycle behavior only; the durable store, real app-server process driver, encrypted process-isolated Credential Capsule, approval bridge, and thread/turn mapping remain implementation work. Claude subscription login is blocked unless its policy decision includes a recorded approval reference.

The supervisor binds one immutable secret-free launch manifest and one opaque capsule reference to a user-owned Provider Connection. At most one Provider Runner Generation is current. Exit callbacks carry generation plus handle identity, and recovery always increments the generation before reopening the capsule. Stop seals the capsule for later recovery; destroy leaves only tombstone metadata and cannot be reversed. Deployment profiles provide abort-aware runtime and capsule drivers without changing this Interface. Detailed contract and failure behavior: `docs/CODEX_RUNNER_SUPERVISOR.md`.
...
**Current Implementation:** protocol v1, registry, defensive manifest discovery, strict/redacted configuration validator, seven capability contracts, generation-fenced profile handle, checkpointed migration executor, reusable conformance harness, and a synthetic full profile exist in `packages/deployment-profile-sdk`. The experimental `packages/profile-aws` package maps all seven contracts to S3, Secrets Manager, DynamoDB, Cognito, CloudWatch Logs, EKS Deployments, and EFS CSI PVCs. It passes PROFILE-001 through PROFILE-011 and AWS-001 through AWS-006 against deterministic fakes, requires a capsule before a Provider Runner, and persists fences in DynamoDB plus Kubernetes annotations. No live AWS conformance or supported cloud profile exists; protocol v1 remains provisional until two materially different profiles pass real-resource qualification. Detailed records: `docs/DEPLOYMENT_PROFILES.md` and `docs/AWS_PROFILE.md`.
...
Codex defaults to app-server device-code login for a remote deployment and may use its managed browser flow when the callback route is safely bound. The current implementation maps device-code auth through a bounded local stdio JSONL transport; it still does not spawn app-server or run agent turns. WebSocket app-server control is excluded because the inspected Interface marks it experimental and unsupported. The inspected device-login response has no expiry field, so the displayed 15-minute challenge deadline is local replay protection, not a provider lifetime claim. Claude subscription login is not enabled in a public build until Anthropic approval is recorded. After approval, the Claude adapter must still use the official login or `claude setup-token` path rather than an OpenCloudOS OAuth client.

App-server can send client-directed requests for command, file, permission, MCP, and other host decisions. The transport rejects every such request with a fixed unsupported error until Capability Broker can bind it to User, Tenant, Workspace, Agent Session, provider turn and item, Prepared Action, expiry, and one auditable decision. Automatic approval is forbidden.
