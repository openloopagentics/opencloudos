...
| OAuth credential | SecretStore adapter | Provider-defined durable write | Provider backup or credential reauthorization |
...
5. Infrastructure adapters: trusted operator-selected data and secret systems.
6. External systems: independently trusted providers with their own authorization and consistency.
...
The first production topology uses one Kubernetes cluster, a replicated PostgreSQL deployment, an object store, an OIDC provider, a secret store, stateless gateways, a shard controller, and multiple runtime-shard StatefulSets. Cross-region operation is disaster recovery, not active-active, until placement and storage replication semantics are proven.
