...
**Design status:** proposed for OpenCloudOS 1.0. The Codex app-server authentication protocol, bounded stdio JSONL transport, and capsule-bound supervisor contract are implemented against synthetic drivers for `codex-cli 0.146.1`; real process launch, encrypted Credential Capsule isolation, approval bridging, and agent-turn execution remain incomplete. Claude subscription support is designed but release-blocked until Anthropic gives written approval for third-party Claude.ai authentication and subscription rate limits.
...
The current code is contract, auth-protocol, transport, and supervisor-lifecycle evidence—not production subscription support. See the [Codex protocol spike](./CODEX_ADAPTER_SPIKE.md), [Provider Runner transport](./CODEX_RUNNER_TRANSPORT.md), [Provider Runner supervisor](./CODEX_RUNNER_SUPERVISOR.md), and [compatibility matrix](./PROVIDER_COMPATIBILITY.md) for the release state.
...
- A recovery creates a new Provider Runner Generation; process exits from an older generation cannot mutate the replacement.
...
3. **Codex vertical slice — active:** bounded stdio transport and SUPERVISOR-001–008 lifecycle contract implemented; next implement the isolated local runtime and encrypted capsule drivers, then connect approved test users through device-code login, run turns, expose limit state, restart runners, and revoke.
